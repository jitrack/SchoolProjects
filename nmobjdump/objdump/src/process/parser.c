...
static inline bool is_a_flag(char const *s)
{
    return (!strcmp(s, "-f") || !strcmp(s, "-s") || !strcmp(s, "-fs") ||
            !strcmp(s, "--file-headers") || !strcmp(s, "--ful-contents"));
}
...
file_p get_all_files(char const * const *argv)
{
    ...
    if (!argv)
        return (NULL);
    for (; *argv; ++argv) {
        if (!is_a_flag(*argv) && add_file(&files, *argv))
            return (NULL);
    }
    return (files);
}
